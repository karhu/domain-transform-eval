                                ...
                                Mat2<float> dIdx, float boxR)
{
    ...
    for (uint i=0; i<H; i++)
    {
        ...
        for (uint j=0; j<W; j++)
        {
            ...
            while (dIdx.data[iW+posL] < dtL && posL < W-1)
            {
//                sum.r -= img.data[i*W+posL].r;
//                sum.g -= img.data[i*W+posL].g;
//                sum.b -= img.data[i*W+posL].b;
                sum.mmvalue = _mm_sub_ps(sum.mmvalue, img.data[iW+posL].mmvalue);
                posL++;
            }
            while (posR < W && dIdx.data[iW+posR] < dtR )  // attention, allows for index = W
            {
//                sum.r += img.data[i*W+posR].r;
//                sum.g += img.data[i*W+posR].g;
//                sum.b += img.data[i*W+posR].b;
                sum.mmvalue = _mm_add_ps(sum.mmvalue, img.data[iW+posR].mmvalue);
                posR++;
            }
